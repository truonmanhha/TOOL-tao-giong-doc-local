...
class OmniVoice(PreTrainedModel):
    ...
    def create_voice_clone_prompt(
        ...
        if preprocess_prompt:
            ...
            from omnivoice.utils.text import normalize_vietnamese_numbers
            is_vi = bool(re.search(r'[áàảãạăắằẳẵặâấầẩẫậéèẻẽẹêếềểễệíìỉĩịóòỏõọôốồổỗộơớờởỡợúùủũụưứừửữựýỳỷỹỵđĐ]', ref_text))
            ref_text = normalize_vietnamese_numbers(ref_text, is_vi=is_vi)
    ...
    def _preprocess_all(
        ...
        # Chuẩn hóa số thập phân và phân cách hàng nghìn tiếng Việt
        from omnivoice.utils.text import normalize_vietnamese_numbers
        normalized_text_list = []
        for i, t in enumerate(text_list):
            is_vi = (language_list[i] == "Vietnamese") or bool(
                re.search(
                    r"[áàảãạăắằẳẵặâấầẩẫậéèẻẽẹêếềểễệíìỉĩịóòỏõọôốồổỗộơớờởỡợúùủũụưứừửữựýỳỷỹỵđĐ]",
                    t,
                )
            )
            norm_t = normalize_vietnamese_numbers(t, is_vi=is_vi)
            if is_vi:
                from omnivoice.utils.text import map_vietnamese_emotions
                norm_t = map_vietnamese_emotions(norm_t)
            normalized_text_list.append(norm_t)
    ...
    def _generate_iterative(
        ...
        for step in range(gen_config.num_step):
            print(f"[STEP_PROGRESS] {step + 1}/{gen_config.num_step}")
            batch_logits = self(
